...
const EmployeeDetails = () => {
  const { id } = useParams();
  const { employees, favorites, addFavorite, removeFavorite } = useContext(AppContext);
  const [employee, setEmployee] = useState(null);

  useEffect(() => {
    const foundEmployee = employees.find(emp => emp.login.uuid === id) || favorites.find(emp => emp.login.uuid === id);
    if (foundEmployee) {
  ...
  console.log(employee.location.coordinates.latitude, employee.location.coordinates.longitude);
